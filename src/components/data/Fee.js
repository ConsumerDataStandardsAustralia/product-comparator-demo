  ...
  return (
    <li>
      <div>
        ...
        {!!accrualFrequency && <span> - every <Duration value={accrualFrequency}/></span>}
      </div>
      <div>
        Fee Type - {translateFeeType(feeType)}
        {feeType === 'PERIODIC' && <span> - every <Duration value={additionalValue}/></span>}
      ...
      {!!currency && <div>Currency - {currency}</div>}
      {
        ...
        <div>
          {isDuration(additionalValue) ? <>every <Duration value={additionalValue}/></> : additionalValue}
        </div>}
